...
from pal_agent.utils.string_utils import contains_punctuation, is_numbered_bullet_list_item
...
def parse_semi_formatted_text(text):
    ...
    if "actions" in result_dict or "action" in result_dict:
        key = "actions" if "actions" in result_dict else "action"
        actions = result_dict[key]
