...
class BaseProvider(metaclass=Singleton):
    ...
    def write(func):
        ...
        def wrapper(self, *args, **kwargs):
            # Wrap the original logger.info method
            original_write = logger.info
            ...
            # Replace logger.info with new_write
            logger.info = new_write
            try:
                result = func(self, *args, **kwargs)
            finally:
                # Restore the original logger.info method
                logger.info = original_write
...
class BaseModuleProvider(BaseProvider):
    ...
    def _extract_keys_from_template(self):
        ...
        logger.info(f"Recommended input parameters: {input_keys}")
        ...
        logger.info(f"Recommended output parameters: {output_keys}")
    ...
    def _check_input_keys(self, params: Dict[str, Any]):
        for key in self.input_keys:
            if key not in params:
                logger.info(f"Key {key} is not in the input parameters")
                params[key] = None
